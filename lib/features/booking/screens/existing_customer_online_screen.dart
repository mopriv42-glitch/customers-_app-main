...
class ExistingCustomerOnlineScreen extends ConsumerStatefulWidget {
  const ExistingCustomerOnlineScreen({super.key});

  @override
  ConsumerState<ExistingCustomerOnlineScreen> createState() =>
      _ExistingCustomerOnlineScreenState();
}

class _ExistingCustomerOnlineScreenState
    extends ConsumerState<ExistingCustomerOnlineScreen>
    with AnalyticsScreenMixin {
  String selectedServiceType = 'حصة خصوصية';
  String selectedSubject = 'الرياضيات';
  String selectedGrade = 'الصف التاسع';
  ...
  @override
  String get screenName => 'ExistingCustomerOnlineScreen';
  ...
  void initState() {
    super.initState();
    // Pre-fill with last lesson data (simulated)
  ...
  Widget build(BuildContext context) {
    ...
    if (!isLoading) {
      ...
      customerBooking.serviceTypeId = 2;
      customerBooking.serviceType = bookingProvider.serviceTypesList.firstWhere(
          ...
          orElse: () => bookingProvider.serviceTypesList.isNotEmpty
              ...
              : ServiceTypeModel(id: 2, serviceType: 'حصة خصوصية'));
  ...
  void _showDatePicker() async {
    logButtonClick('show_date_picker');
  ...
  void _showTimePicker() async {
    logButtonClick('show_time_picker');
    ...
    if (time != null) {
      ...
      logStep('time_selected', data: {
        'selected_time': "${time.hour}:${time.minute}",
      });
    }
  }

  void _showAltTimePicker() async {
    logButtonClick('show_alt_time_picker');
    final time = await showTimePicker(
      context: context,
    ...
    if (time != null) {
      ...
      customerBooking.altTime =
          "${time.hour.toString().padLeft(2, '0')}:${time.minute.toString().padLeft(2, '0')}";
      logStep('alt_time_selected', data: {
        'selected_alt_time': "${time.hour}:${time.minute}",
      });
    }
  }

  void _navigateToConfirmation() async {
    logButtonClick('next_button_clicked', data: {
    ...
    if (mounted && result) {
      ...
      logStep('booking_created_successfully', data: {
        'booking_id': booking.id,
        'price': booking.price,
      });
      context.push('/existing-customer-online-confirmation', extra: {
