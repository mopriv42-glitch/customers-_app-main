...
class MatrixChatProvider extends ChangeNotifier {
  ...
  /// Login to Matrix using credentials from the server response
  /// 
  /// The server provides the Matrix user ID and we use the known default password
  /// since the server also sets this password when provisioning the Matrix user.
  /// 
  /// This is more reliable than token-based auth since the Matrix SDK v1.1.1
  /// doesn't have direct support for initializing with a pre-obtained access token.
  Future<bool> loginWithAccessToken({
    ...
    try {
      ...
      // Parse homeserver URL
      final homeserverUri = Uri.parse(homeserver);

      debugPrint('Matrix: Attempting login for user: $userId');
      debugPrint('Matrix: Homeserver: $homeserver');
      ...
      // Use password-based login with the known default password
      // The server sets this password for all provisioned users
      const defaultPassword = 'Private4T@@2024';

      debugPrint('Matrix: Logging in with password for user: $userId');

      // Use standard password login - this is proven to work
      final identifier = AuthenticationUserIdentifier(user: userId);
      await c.login(
        LoginType.mLoginPassword,
        identifier: identifier,
      ...
      if (_isLoggedIn) {
        debugPrint('Matrix: Login successful for user: $userId');
        ...
        debugPrint('Matrix: Login completed but isLogged() returned false');
        return false;
      }
    } catch (e) {
      debugPrint('Matrix: Error during login: $e');
      final msg = e.toString();
      if (msg.contains('M_FORBIDDEN')) {
        debugPrint('Matrix: Invalid credentials');
      }
      return false;
    }
  }
